...
img = cv2.imread('Image/shifamutia.jpg',0)
img2 = img.copy()
template = cv2.imread('Image/shifamutia2.jpg',0)
...
einstein = cv2.imread('Image/shifamutia2.jpg')
einstein = cv2.cvtColor(einstein, cv2.COLOR_BGR2RGB)  # Simpan dengan nama yang sama

# Panggil dan konversi warna agar sesuai dengan Matplotlib
solvay = cv2.imread('Image/shifamutia.jpg')  # Perbaiki penulisan nama gambar
solvay = cv2.cvtColor(solvay, cv2.COLOR_BGR2RGB)

plt.subplot(121), plt.imshow(einstein), plt.title('ShifaMutia')
plt.subplot(122), plt.imshow(solvay), plt.title('fotbar')
plt.show()